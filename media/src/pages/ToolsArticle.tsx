...
export default function ToolsArticle() {
  ...
  const callCms = React.useCallback(async (path: string, body: any) => {
  ...
  }, [ADMIN_TOKEN, CMS_BASE])

  const callPhp = React.useCallback(async (path: string, body: any) => {
  ...
  const callWithFallback = React.useCallback(
    async (
      path: string,
      body: any,
      ...
      let phpIssue: string | null = null
      try {
        const first = await callPhp(path, body)
        if (validate(first)) return first
        if (first && typeof first?.error === 'string' && first.error.trim()) {
          phpIssue = first.error.trim()
        } else {
          phpIssue = 'PHP経由のレスポンスが無効でした'
        }
      } catch (err: any) {
        phpIssue = err?.message || 'PHP経由の呼び出しに失敗しました'
      }

      try {
        const fallback = await callCms(path, body)
        if (validate(fallback)) return fallback
        const detail = typeof fallback?.error === 'string' && fallback.error.trim()
          ? fallback.error.trim()
          : ''
        const message = detail ? `${fallbackMessage}: ${detail}` : fallbackMessage
        throw new Error(message)
      } catch (err: any) {
        const cmsMessage = err?.message || fallbackMessage
        if (phpIssue) {
          throw new Error(`${cmsMessage}（PHP経由も失敗: ${phpIssue}）`)
        }
        throw new Error(cmsMessage)
      }
    },
    [callCms, callPhp],
  )
  ...
  async function onGenerateOutline() {
    ...
    if (!sources.length) {
      ...
      try {
        ...
        const hasResults = (value: any) => Array.isArray(value?.results) && value.results.length > 0
        if (!hasResults(res)) {
        ...
        const list: SourceResult[] = hasResults(res) ? res.results : []
